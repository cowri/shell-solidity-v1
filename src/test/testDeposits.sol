...
contract SelectiveDepositTest is Setup, DSMath, DSTest {
    ...
    // //     uint256 viewShells = viewDeposit(1, dai, 90*WAD, usdc, 90*(10**6), usdt, 90*(10**6), susd, 30*WAD);
    // //     uint256 newShells = deposit(1, dai, 90*WAD, usdc, 90*(10**6), usdt, 90*(10**6), susd, 30*WAD);
    // //     // l1.swapByOrigin(susd, usdc, 28*(10**17), 0, now+50);
    // //     // l1.swapByOrigin(susd, usdc, 28*(10**17), 0, now+50);
    // //     // l1.swapByOrigin(susd, usdc, 28*(10**17), 0, now+50);
    // //     // uint256 newShells = deposit(1, dai, 0, usdc, 0, usdt,  0, susd, 0);
    // //     // assertEq(newShells, 32483751922977906373);
    // //     emit log_named_uint("viewShells", viewShells);
