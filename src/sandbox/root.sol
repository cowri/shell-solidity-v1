...
library Root {
  ...
  function toUInt128 (bytes16 x) internal pure returns (uint128) {
    uint256 exponent = uint128 (x) >> 112 & 0x7FFF;
  ...
  event ping(bytes16);
  event iping(bytes32, int256);
  event uping(bytes32, uint256);
  event bping(bytes32, bool);
  ...
  function find_root (bytes16 guess, bytes16 base, uint256 root, uint256 max_iterations) internal  returns (bytes16) {

    for (uint i = 0; i < max_iterations; i++) {

      bytes16 divisor_rhs = guess;
      bytes16 divisor_lhs = fromUInt(root);
      for (uint j = 1; j < root - 1; j++) divisor_rhs = mul(divisor_lhs, guess);
      bytes16 dividend_lhs = mul(divisor_rhs, guess);

      if (eq(dividend_lhs, base)) return guess;

      bytes16 dividend = sub(dividend_lhs, base);

      int256 idividend = toInt(mul(bytes16(0x403abc16d674ec800000000000000000), dividend));

      // emit iping("idividend", idividend);

      bytes16 divisor = mul(divisor_lhs, divisor_rhs);

      bytes16 rhs_quotient = div(dividend, divisor);
      int256 i_quotient = toInt(mul(rhs_quotient, bytes16(0x403abc16d674ec800000000000000000)));
      // emit iping("i_quotient", i_quotient);
  ...
  // function newton (bytes16 guess, bytes16 base, uint256 root, uint256 max_iterations) internal pure returns (bytes16) {
