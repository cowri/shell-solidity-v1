...
contract RootCalculator is Utils {
    ...
    */
    ...
    function power(uint256 _baseN, uint256 _baseD, uint32 _expN, uint32 _expD) public returns (uint256, uint8) {
        require(_baseN < MAX_NUM);

        uint256 baseLog;
        uint256 base = _baseN * FIXED_1 / _baseD;
        emit log_uint("base", base);
        ...
        emit log_uint("baseLog", baseLog);

        uint256 baseLogTimesExpN = baseLog * _expN;

        emit log_uint("baseLog * _expN", baseLogTimesExpN);

        uint256 baseLogTimesExpNDivExpD = baseLogTimesExpN / _expD;

        emit log_uint("baseLogTimesExpN / _expD", baseLogTimesExpNDivExpD);
        if (baseLogTimesExpNDivExpD < OPT_EXP_MAX_VAL) {
            return (optimalExp(baseLogTimesExpNDivExpD), MAX_PRECISION);
        }
        else {
            uint8 precision = findPositionInMaxExpArray(baseLogTimesExpNDivExpD);
            return (generalExp(baseLogTimesExpNDivExpD >> (MAX_PRECISION - precision), precision), precision);
        }
    }
