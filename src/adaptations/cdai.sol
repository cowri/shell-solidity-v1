...
contract CDAIAdaptation is DSMath {
     ...
     */
    function wrap (uint256) public returns (uint256) {
     ...
     */
    function unwrap (uint256) public returns (uint256) {
     ...
     */
    function getNumeraireAmount (uint256) public returns (uint256) {
        uint256 rate = CTokenI(address(0)).currentRates();
    }
