...
// contract ViewSelectiveDepositTest is LoihiSetup, DSMath, DSTest {
...
//     function viewDeposit (address waddr, uint256 wamount, address xaddr, uint256 xamount, address yaddr, uint256 yamount, address zaddr, uint256 zamount) public returns (uint256) {
//         address[] memory addrs = new address[](4);
//         uint256[] memory amounts = new uint256[](4);
...
//         return l1.viewSelectiveDeposit(addrs, amounts);
//     }
...
//         uint256 newShells = viewDeposit(dai, 10*WAD, usdc, 10 * (10**6), usdt,  10 * (10**6), susd, 2500000000000000000);
...
//         uint256 newShells = viewDeposit(dai, 5 * WAD, usdc, 1 * (10**6), usdt, 3 * (10**6), susd, WAD);
//         assertEq(newShells, 9995000591686018553);
//         emit log_named_uint("startingShells", startingShells);
//     }

//     function testFromZeroPartialUpperSlippage145Dai90Usdc90Usdt50Susd () public {
//         uint256 newShells = viewDeposit(dai, 145 * WAD, usdc, 90 * (10**6), usdt, 90 * (10**6), susd, 50 * WAD);
...
//         uint256 newShells = viewDeposit(dai, 95 * WAD, usdc, 55 * (10**6), usdt, 95 * (10**6), susd, 15 * WAD);
...
//         uint256 newShells = viewDeposit(dai, 5 * WAD, usdc, 5 * (10**6), usdt, 70 * (10**6), susd, 28 * WAD);
...
//         uint256 newShells = viewDeposit(dai, WAD, usdc, 51 * (10**6), usdt, 51 * (10**6), susd, WAD);
...
//         uint256 startingShells = l1.proportionalDeposit(300*WAD);
//         uint256 newShells = viewDeposit(dai, WAD/1000, usdc, 90*(10**6), usdt, 90*(10**6), susd, 0);
...
//         uint256 newShells = viewDeposit(dai, 0, usdc, 46*(10**6), usdt, 53*(10**6), susd, 0);
...
//         uint256 startingShells = deposit(1, dai, 145*WAD, usdc, 90*(10**6), usdt, 90*(10**6), susd, 50*WAD);
//         uint256 newShells = viewDeposit(dai, WAD, usdc, 46*(10**6), usdt, 53*(10**6), susd, WAD);
...
//         uint256 newShells = viewDeposit(dai, 0, usdc, 36*(10**6), usdt, 0, susd, 18*WAD);
...
//         uint256 newShells = viewDeposit(dai, 0, usdc, 5*(10**6), usdt, 0, susd, 3*WAD);
...
//         uint256 newShells = viewDeposit(dai, 12*WAD, usdc, 12*(10**6), usdt, 10**6, susd, WAD);
...
//         uint256 startingShells = deposit(1, dai, 95*WAD, usdc, 95*(10**6), usdt, 55*(10**6), susd, 15*WAD);
//         uint256 newShells = viewDeposit(dai, 9*WAD, usdc, 9*(10**6), usdt, 0, susd, 0);
...
//         uint256 newShells = viewDeposit(dai, 5*WAD, usdc, 5*(10**6), usdt, 0, susd, 0);
...
//         uint256 startingShells = deposit(1, dai, 145*WAD, usdc, 90*(10**6), usdt, 90*(10**6), susd, 50*WAD);
//         uint256 newShells = viewDeposit(dai, 8*WAD, usdc, 12*(10**6), usdt, 10*(10**6), susd, 2*WAD);
...
//         uint256 newShells = viewDeposit(dai, 5*WAD, usdc, 5*(10**6), usdt, 5*(10**6), susd, 2*WAD);
...
//         uint256 startingShells = l1.proportionalDeposit(300*WAD);
//         uint256 shellsMinted = viewDeposit(dai, 36*WAD, usdc, 0, usdt, 0, susd, 0);
...
//         uint256 startingShells = l1.proportionalDeposit(300*WAD);
//         uint256 shellsMinted = viewDeposit(dai, 36*WAD + WAD/10000, usdc, 0, usdt, 0, susd, 0);
...
//         uint256 startingShells = deposit(1, dai, 55*WAD, usdc, 95*(10**6), usdt, 95*(10**6), susd, 15*WAD);
//         uint256 newShells = viewDeposit(dai, 105*WAD, usdc, 0, usdt, 0, susd, 37*WAD);
...
//         uint256 startingShells = deposit(1, dai, 90*WAD, usdc, 145*(10**6), usdt, 90*(10**6), susd, 50*WAD);
//         uint256 newShells = viewDeposit(dai, 165*WAD, usdc, 0, usdt, 165*(10**6), susd, 0);
...
//         uint256 startingShells = deposit(1, dai, 90*WAD, usdc, 145*(10**6), usdt, 90*(10**6), susd, 50*WAD);
//         uint256 newShells = viewDeposit(dai, 165*WAD, usdc, (10**6)/10000, usdt, 165*(10**6), susd, WAD/2);
...
//         viewDeposit(dai, 100*WAD, usdc, 0, usdt, 0, susd, 0);
//     }

//     function testFailDepositLowerHaltCheck30Pct () public {
//         l1.proportionalDeposit(300*WAD);
//         viewDeposit(dai, 300*WAD, usdc, 0, usdt, 300*(10**6), susd, 100*WAD);
//     }

//     function testFailDepostUpperHaltCheck10Pct () public {
//         l1.proportionalDeposit(300*WAD);
//         viewDeposit(dai, 0, usdc, 0, usdt, 0, susd, 500*WAD);
//     }

//     function testFailDepositLowerHaltCheck10Pct () public {
//         l1.proportionalDeposit(300*WAD);
//         viewDeposit(dai, 200*WAD, usdc, 200*(10**6), usdt, 200*(10**6), susd, 0);
//     }
